...
import { Users, Shield, ShieldAlert, RefreshCw, Mail, UserCheck, UserX } from 'lucide-react';
import { UserProfile } from '../types';
import { getAllProfiles, updateUserRole } from '../services/supabaseService';
...
const UserManagement: React.FC = () => {
  ...
  const handleToggleRole = async (profile: UserProfile) => {
    const newRole = profile.role === 'super_admin' ? 'operador' : 'super_admin';
    const confirmMsg = `Deseja alterar o cargo de ${profile.full_name} para ${newRole === 'super_admin' ? 'Administrador' : 'Operador'}?`;
            ...
            <tbody className="divide-y divide-slate-800/50">
              {profiles.map((profile) => (
                <tr key={profile.id} className="hover:bg-slate-800/30 transition-colors group">
                  ...
                  <td className="px-8 py-6">
                    <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${profile.role === 'super_admin' ? 'bg-amber-500/10 text-amber-500 border border-amber-500/20' : 'bg-slate-800 text-slate-400 border border-slate-700'}`}>
                      {profile.role === 'super_admin' ? <ShieldAlert size={12} /> : <Shield size={12} />}
                      {profile.role === 'super_admin' ? 'Administrador' : 'Operador'}
                    </div>
                  </td>
                  <td className="px-8 py-6 text-right">
                    <button 
                      onClick={() => handleToggleRole(profile)}
                      className={`p-3 rounded-xl transition-all ${profile.role === 'super_admin' ? 'bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white' : 'bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-white'}`}
                      title={profile.role === 'super_admin' ? 'Remover Admin' : 'Tornar Admin'}
                    >
                      {profile.role === 'super_admin' ? <UserX size={20} /> : <UserCheck size={20} />}
                    </button>
                  </td>
                </tr>
              ))}
